...
- The five samples are generated in `site-samples.js`, stereo at 44.1 kHz, each at about −16 LUFS: chime (an airport chime pair, C4–E4–G4–C5 rising, a pause, then falling, on vibraphone bars with their upper modes and a mallet's click), canon (Pachelbel's Canon in D: the ground bass on a cello, two violins in canon, bowed from detuned saws), prelude (Bach's Prelude in C, BWV 846, bars 1–4, on an FM electric piano), toccata (the opening of Bach's Toccata in D minor, BWV 565, on a pipe organ's full chorus in a church) and mountainking (Grieg's In the Hall of the Mountain King, bars 1–4, on 8-bit pulse, triangle and noise voices, twice, speeding up, with drums the second time). A Freeverb hall places each. No recording is fetched or uploaded.
...
| `site: names and numbers light under the pointer; a name lists its choices: samples and a file, or downloads` | Hover a number and the sample name → click the sample name → the second sample → click the save name → its .mp3 → the list again, Tab → type the third sample, drums, a file name → Open a file… | Lit tokens use the selection's gray; no buttons ride the program; the samples and the downloads list; the chosen sample keeps the edits and names the download; a chosen format writes its extension and downloads, Tab leaves without one; typing a sample switches; an unknown sample and a quoted name over a sample are refused; Open a file… opens the picker |
