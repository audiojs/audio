...
- The five samples are generated in `site-samples.js`: a few seconds each of sparse, distinct strikes that read in the waveform, stereo at 44.1 kHz and about −16 LUFS. chime (an airport chime pair, C4–E4–G4–C5 rising, a pause, then falling, on vibraphone bars with their upper modes and a mallet's click), handpan (a slow phrase in D Kurd, each note tuned to its octave and twelfth, the ding ringing along), bowl (a Himalayan singing bowl struck once, its modes at 1, 2.74, 5.13, 8.21 and 11.9 times the note, each a beating pair), rhodes (Dm9, G13 and Cmaj9 on an FM electric piano with the suitcase's tremolo and a tape's wobble) and heartbeat (lub-dub at 66 beats a minute). A Freeverb hall places the tonal ones. No recording is fetched or uploaded.
