...
- `window-function.js` and `periodic-function.js` bundle the installed window-function 3.0.3 and periodic-function 2.1.2 (both MIT)
  ...
  to the waveform. `logo.html` is its lab, any signal through any window. `logo-motion.js` makes it answer the hand, as
  the header will: it drifts, stirs when hovered, turns under a drag and spins on, and a tap changes its waveform;
  `logo-motion.html` tries its settings. `node .site-build.js` refreshes the bundles too.
